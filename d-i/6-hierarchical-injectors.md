...
- все возможные варианты были указаны в примерах можете посмотреть примеры и вы все поймете
- если хотите чтоб все разъяснили по полочкам читает сам сайт

**ElementInjector use case examples**

- В чем хорош **Dependency Injector** ?
    - хорош тем что если мы переопределяем сам сервис на одном из уровня `ElementInjector` все его зависимости которые
      находятся в конструкторе, будут доставаться там откуда они доступны. Либо `current ElementInjector` либо уже будет
      искать в прародителях.
    - Потому что создания инстанции зависимостей идет каждый раз по одному и тому же механизму `Dependency Injector`
    - и большой здесь плюс в том что если мы теперь хотим переопределить внутреннюю зависимость которые лежат в
      конструкторе. Нам всего лишь нужно заново эту зависимость переопределить в `ElementInjector` через `providers`
      , `viewProviders`,
    - выходит что внутренняя зависимость будет заново создана новая инстанция на уровне `ElementInjector`.


- еще одна особенность **Dependency Injector**
    - если мы хотим у сервиса внутренние зависимости переопределить для какого то `ElementInjector-a`. то нам все равно
      необходимо заново этот ж сервис определить в `providers`, `viewProviders`. Если мы сам сервис не определим в самом
      уровне `ElementInjector-a`. то те зависимости которые указали в `providers`, `viewProviders` они не будут
      использованы просто по одной причине. Не будет создан новая инстанция сервиса для   `ElementInjector-a` а просто
      возьмет тот что выше в прародителях. а то что сервис выше в прародителях никак не знает о существования
      зависимостей ниже по уровню. Выходит что раз мы хотим переопределить внутренние зависимости сервиса то нам нужно и
      сам сервис переопределить. Переопределить значит просто указать в `providers`
